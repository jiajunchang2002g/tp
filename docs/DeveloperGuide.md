...
**4. Add challenge for `clear`command**

Currently, the `clear` command can be executed without confirmation, which may lead to accidental data loss. A future version will add a confirmation step that prompts the user to type a randomly generated challenge string E.g. `ABC123` before proceeding with the clear operation. This will help prevent unintended deletions while still allowing power users to execute the command efficiently when needed.
